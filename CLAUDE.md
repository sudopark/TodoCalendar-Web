...
- 테스트 이름은 "무엇을 하면 어떤 결과가 나오는가"로 작성 (HOW가 아니라 WHAT/WHY)
- 내부 구현 검증 금지: 특정 함수가 몇 번 호출됐는지, 어떤 인자로 호출됐는지 검증하지 않는다
- 외부 의존성(Firebase, API)은 그 역할(인증, 데이터 조회)만 모킹하고, 상태/결과로 검증한다
