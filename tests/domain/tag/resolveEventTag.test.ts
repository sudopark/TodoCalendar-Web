...
import { resolveEventTag } from '../../../src/domain/tag/resolveEventTag'
...
describe('resolveEventTag', () => {
  ...
  it('id가 HOLIDAY_TAG_ID면 kind=holiday, 공휴일 색상을 사용한다', () => {
  ...
  it('id가 DEFAULT_TAG_ID면 kind=default, 디폴트 색상을 사용한다', () => {
  ...
  it('id가 null이고 defaultTagId가 없으면 kind=default + 디폴트 색상이 반환된다', () => {
    const r = resolveEventTag(null, ctx({ defaultColors: { default: '#123456', holiday: '#abcdef' } }))
    expect(r.kind).toBe('default')
    expect(r.color).toBe('#123456')
  })

  it('id가 null이고 defaultTagId/defaultColors 모두 없으면 kind=none + 폴백 회색이 반환된다', () => {
  ...
  it('태그 색상이 비어있으면(tag.color_hex == null) 폴백 회색으로 채워진다', () => {
