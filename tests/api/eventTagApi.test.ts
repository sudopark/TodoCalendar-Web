...
describe('eventTagApi', () => {
  ...
  it('getAllTags()가 /v2/tags/all로 GET 호출한다', async () => {
    const { eventTagApi } = await import('../../src/api/eventTagApi')
    await eventTagApi.getAllTags()

    const [url, options] = fetchSpy.mock.calls[0]
    expect(String(url)).toContain('/v2/tags/all')
    expect((options as RequestInit).method).toBe('GET')
  })

  it('createTag(body)가 /v2/tags/tag로 POST 호출한다', async () => {
    ...
    const [url, options] = fetchSpy.mock.calls[0]
    expect(String(url)).toContain('/v2/tags/tag')
    expect((options as RequestInit).method).toBe('POST')
    expect(JSON.parse((options as RequestInit).body as string)).toEqual(body)
  })

  it('updateTag(id, body)가 /v2/tags/tag/:id로 PUT 호출한다', async () => {
    fetchSpy.mockResolvedValue(
    ...
    const [url, options] = fetchSpy.mock.calls[0]
    expect(String(url)).toContain('/v2/tags/tag/tag-1')
    expect((options as RequestInit).method).toBe('PUT')
    expect(JSON.parse((options as RequestInit).body as string)).toEqual(body)
  })

  it('deleteTag(id)가 /v2/tags/tag/:id로 DELETE 호출한다', async () => {
    fetchSpy.mockResolvedValue(
  ...
  it('deleteTagAndEvents(id)가 /v2/tags/tag_and_events/:id로 DELETE 호출한다', async () => {
    fetchSpy.mockResolvedValue(
      new Response(JSON.stringify({ status: 'ok' }), { status: 200, headers: { 'content-type': 'application/json' } })
    )
    ...
    const [url, options] = fetchSpy.mock.calls[0]
    expect(String(url)).toContain('/v2/tags/tag_and_events/tag-1')
    expect((options as RequestInit).method).toBe('DELETE')
  })
})
