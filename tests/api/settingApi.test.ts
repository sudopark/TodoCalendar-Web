...
describe('settingApi', () => {
  ...
  it('getDefaultTagColors()가 /v2/setting/event/tag/default/color로 GET 호출한다', async () => {
    const { settingApi } = await import('../../src/api/settingApi')
    await settingApi.getDefaultTagColors()

    const [url, options] = fetchSpy.mock.calls[0]
    expect(String(url)).toContain('/v2/setting/event/tag/default/color')
    expect((options as RequestInit).method).toBe('GET')
  })

  it('updateDefaultTagColors(body)가 /v2/setting/event/tag/default/color로 PATCH 호출한다', async () => {
    const { settingApi } = await import('../../src/api/settingApi')
