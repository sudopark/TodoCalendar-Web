...
// 기존 Test 3/4는 팝오버 기반 별도 시나리오로 재작성이 필요하며 이 브랜치 스코프 외.
test.skip('DayEventList에서 일정을 클릭하면 이벤트 상세 오버레이가 열리고 이름이 표시된다', async ({ page }) => {
...
// ─────────────────────────────────────────────────────────────────────────────
test.skip('이벤트 상세에서 편집 버튼을 클릭하면 편집 폼으로 이동하고, 삭제하면 메인 페이지로 돌아온다', async ({ page }) => {
