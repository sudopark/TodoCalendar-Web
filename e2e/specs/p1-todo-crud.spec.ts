...
test('Todo 이름을 입력하고 저장하면 팝오버가 닫힌다', async ({ page }) => {
  ...
  await page.route('**/v2/todos/todo', async route => {
...
test('Todo 수정 폼 진입 시 "Todo 수정" 제목이 표시된다', async ({ page }) => {
  ...
  await page.route('**/v2/todos/todo/test-todo-id-edit', async route => {
...
test('수정 폼에서 삭제 버튼 누르면 확인 다이얼로그가 표시된다', async ({ page }) => {
  ...
  await page.route('**/v2/todos/todo/todo-to-delete', async route => {
    if (route.request().method() === 'GET') {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
...
test('삭제 다이얼로그에서 취소를 누르면 다이얼로그가 닫힌다', async ({ page }) => {
  ...
  await page.route('**/v2/todos/todo/todo-cancel-delete', async route => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
...
test('삭제 다이얼로그에서 확인하면 API를 호출하고 이전 페이지로 돌아간다', async ({ page }) => {
  ...
  await page.route('**/v2/todos/todo/todo-confirm-delete', async route => {
    if (route.request().method() === 'GET') {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
