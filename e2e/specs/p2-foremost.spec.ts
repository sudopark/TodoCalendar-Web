...
test('GET /v2/foremost/event 가 이벤트를 반환하면 메인 페이지에 배너가 표시된다', async ({ page }) => {
  // given — foremost API가 이벤트를 반환하도록 모킹
  await page.route('**/v2/foremost/event', async route => {
...
test('GET /v2/foremost/event 가 404를 반환하면 배너가 표시되지 않는다', async ({ page }) => {
...
test('GET /v2/foremost/event 가 event: null 을 반환하면 배너가 표시되지 않는다', async ({ page }) => {
