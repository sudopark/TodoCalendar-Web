...
test('TagFilter에서 태그를 클릭하면 해당 태그의 Todo가 Current 목록에서 사라지고, 다시 클릭하면 재등장한다', async ({ page }) => {
  ...
  // CalendarList의 "업무" 토글 버튼 클릭 → 숨김
  const tagToggleBtn = page.getByRole('button', { name: '업무 숨기기' })
  await expect(tagToggleBtn).toBeVisible()

  // when — 첫 번째 클릭 (숨김)
  await tagToggleBtn.click()

  // then — Todo가 Current 섹션에서 사라진다
  await expect(currentSection.getByText('필터될 Todo')).not.toBeVisible()

  // when — 두 번째 클릭 (다시 표시) — 숨긴 후 aria-label이 "업무 표시"로 변경됨
  const showBtn = page.getByRole('button', { name: '업무 표시' })
