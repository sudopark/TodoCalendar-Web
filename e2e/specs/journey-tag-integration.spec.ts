...
test('Todo 폼에서 "업무" 태그를 선택하고 저장하면 Current 목록에서 태그 색상 점이 표시된다', async ({ page }) => {
  ...
  // then — 저장 후 팝오버가 닫히고 Current Todo 섹션에 todo 가 표시된다
  ...
  // 첫 GET 모킹과 EventRepository.createTodo 의 cache addTodo 가 겹쳐 동일 항목이 두 번 렌더될 수 있어 .first() 로 한정
  await expect(currentTodoList.getByText('업무 태그 Todo').first()).toBeVisible()

  // CurrentTodoRow 의 타임라인 dot — rounded-full 색상 점이 todo 행 안에 있어야 한다
  const todoRow = currentTodoList.locator('div').filter({ hasText: '업무 태그 Todo' }).first()
  await expect(todoRow.locator('.rounded-full').first()).toBeVisible()
})
...
test('TagFilter에서 태그를 클릭하면 해당 태그의 Todo가 Current 목록에서 사라지고, 다시 클릭하면 재등장한다', async ({ page }) => {
  ...
  // Current Todo 섹션에 todo 가 보여야 한다 — 캘린더 셀에도 todo 가 노출될 수 있으니 testid 영역으로 한정
  const currentTodoList = page.getByTestId('current-todo-list')
  await expect(currentTodoList.getByText('필터될 Todo')).toBeVisible()

  // CalendarList(사이드바) 의 "업무" 태그 토글 버튼 — button 의 title/접근 가능 이름으로 식별
  const tagItem = page.getByRole('button', { name: '업무', exact: true })
  await expect(tagItem).toBeVisible()

  // when — 첫 번째 클릭 (숨김)
  await tagItem.click()

  // then — Current Todo 섹션에서 사라진다
  await expect(currentTodoList.getByText('필터될 Todo')).not.toBeVisible()
