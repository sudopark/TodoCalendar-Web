...
test('완료된 항목을 클릭하면 상세 팝오버가 열린다', async ({ page }) => {
  ...
  await page.route('**/v2/event_details/done/done-detail', async route => {
...
test('상세 팝오버의 닫기 버튼을 누르면 팝오버가 닫히고 ArchivePanel 은 그대로 유지된다', async ({ page }) => {
  ...
  await page.route('**/v2/event_details/done/done-close', async route => {
...
test('상세 팝오버의 되돌리기를 누르면 API 호출 + ArchivePanel 에서 항목이 사라진다', async ({ page }) => {
  ...
  await page.route('**/todos/dones**', async route => {
    ...
    if (url.includes('/revert')) {
      reverted = true
      // revertDoneTodo 응답 형태 — { todo, detail? } (RevertDoneTodoResponse)
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
          todo: { uuid: 'todo-revert-detail', name: '되돌리기 상세', is_current: true },
          detail: null,
        }),
      })
      return
  ...
  await page.route(`**/v2/event_details/done/${doneId}`, async route => {
    await route.fulfill({ status: 200, contentType: 'application/json', body: 'null' })
  })
