...
test('완료 목록에서 되돌리기를 클릭하면 항목이 완료 목록에서 사라진다', async ({ page }) => {
  ...
  await page.route('**/v1/todos**', async route => {
    ...
    if ((url.includes('/cancel') || url.includes(`/dones/${doneId}/revert`)) && method === 'POST') {
      reverted = true
      // BFF cancelDoneTodo 응답 형태 — { reverted, done_id }
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ reverted: revertedTodo, done_id: doneId }) })
      return
    }
