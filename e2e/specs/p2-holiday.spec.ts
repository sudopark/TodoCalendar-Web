...
test('GET /v2/holiday 가 공휴일을 반환하면 해당 날짜가 캘린더에 빨간색으로 표시된다', async ({ page }) => {
  // given — 공휴일 API가 4월 5일을 공휴일로 반환
  await page.route('**/v2/holiday**', async route => {
    await route.fulfill({
      status: 200,
  ...
  // then — 4월 5일 날짜 숫자가 강조 색상(text-danger 토큰) 텍스트 클래스를 갖는다
  // MainCalendarGrid 는 holiday/일요일/토요일 강조에 text-danger 를 사용
  const holidayCell = page.getByTestId('day-cell').filter({
    has: page.locator('div.text-danger', { hasText: '5' }),
  }).first()
...
test('GET /v2/holiday 가 빈 목록을 반환하면 일요일을 제외한 날짜는 빨간색이 아니다', async ({ page }) => {
  // given — 공휴일 없음
  await page.route('**/v2/holiday**', async route => {
    await route.fulfill({
      status: 200,
