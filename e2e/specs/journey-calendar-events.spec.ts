...
test('특정 날짜에 이벤트가 있으면 캘린더 셀에 색상 인디케이터가 표시된다', async ({ page }) => {
  ...
  const todayCellWithEvents = page.locator('[data-testid="day-cell"][data-today]').first()
...
test('이벤트가 있는 날짜를 클릭하면 DayEventList에 해당 이벤트가 표시된다', async ({ page }) => {
  ...
  // then — DayEventList에 일정 이름이 표시된다
  await expect(page.getByText('날짜 클릭 테스트 일정')).toBeVisible()
})
...
test('이벤트가 없는 날짜를 클릭하면 "이벤트가 없습니다" 메시지가 표시된다', async ({ page }) => {
  ...
  // 오늘이 아닌 날짜 셀 클릭 (오늘 셀 클릭 시 토글로 패널이 닫히므로)
  const otherCell = page.locator('[data-testid="day-cell"]:not([data-today])').filter({
    hasText: '15',
  }).first()
...
// ─────────────────────────────────────────────────────────────────────────────
// 다음 달 이동 시 fetch가 발생하지 않는 기존 회귀. 별도 이슈에서 조사 필요.
test.skip('다음 달로 이동하면 해당 월 범위로 이벤트 fetch 요청이 발생한다', async ({ page }) => {
  // 다음 달 이벤트
  ...
  const nextBtn = page.getByRole('button', { name: 'Next month', exact: true })
  await nextBtn.click()
