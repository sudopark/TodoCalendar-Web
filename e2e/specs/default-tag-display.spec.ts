...
test('유저가 디폴트 태그를 지정하면 태그 없이 저장된 Todo에 해당 태그명이 표시된다', async ({ page }) => {
  ...
  // getCurrentTodos() → GET /v2/todos (쿼리파라미터 없음, is_current=true 항목 반환)
  ...
  // 나머지 라우트는 빈 응답
  await page.route('**/v2/todos/uncompleted', async route => {
...
test('서버 디폴트 색상이 비어있어도 이벤트는 정상 렌더되고 앱 내장 기본 색상/이름이 노출된다', async ({ page, context }) => {
  ...
  await page.route('**/v2/todos/uncompleted', async route => { await route.fulfill({ status: 200, contentType: 'application/json', body: '[]' }) })
  await page.route('**/v2/schedules**', async route => {
  ...
  await page.route('**/v2/foremost**', async route => { await route.fulfill({ status: 404, body: '{}' }) })
  await page.route('**/v2/holidays**', async route => { await route.fulfill({ status: 200, contentType: 'application/json', body: '[]' }) })
